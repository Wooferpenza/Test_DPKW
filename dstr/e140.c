...
void GET_DATA_MEMORY_E140(ldevusb *dev, u16 *Data, u32 Count, u16 Addr)
{
   ...
   for(i = 0; i<s; i++)
   {
      status = usb_control_msg(dev->udev, usb_rcvctrlpipe(dev->udev, 0),
                     V_GET_ARRAY_E140, USB_TYPE_VENDOR | USB_RECIP_DEVICE | USB_DIR_IN,
...
void PUT_DATA_MEMORY_E140(ldevusb *dev, u16 *Data, u32 Count, u16 Addr) // how words
{
   ...
   for(i = 0; i<s; i++)
   {
      ...
      status = usb_control_msg(dev->udev, usb_sndctrlpipe(dev->udev, 0),
                     V_PUT_ARRAY_E140, USB_TYPE_VENDOR | USB_RECIP_DEVICE | USB_DIR_OUT,
                 //(u16)(Addr+i*2048), 0, (u8 *)(&Data[2048*i]), 2048*sizeof(u16), HZ*10);
                 (u16)(Addr+i*2048), 0, (u8 *)Dat, 2048*sizeof(u16), HZ*10);
   }
...
{
   PUT_PM_WORD_E140(dev,L_FLASH_ENABLED_E140,(u8)Flag);
   return 1;
}
...
{
   *Data = GET_DM_WORD_E140(dev,FlashAddress);
}
...
{
   PUT_DM_WORD_E140(dev,FlashAddress, Data);
}
...
void EnableE140(ldevusb *dev, int b)
{
   ...
   if(b) // start ADC
   {
    ...
    DbgPrint("NCh  %d\n", dev->adcPar.t3.NCh);     
    DbgPrint("Kadr  %d\n", dev->adcPar.t3.Kadr);
    DbgPrint("Stype  %d\n", dev->adcPar.t3.SynchroType);
   
      // pack adc parameters
      for(i=0;i<128;i++) ap.t1.Chn[i] = (u8)MAKE_E140CHAN(dev->adcPar.t3.Chn[i]);
      ...
      COMMAND_E140(dev, V_START_ADC_E140);

class DaqE140: public LDaqBoard
{
public:
   // Base functions
   IFC(ULONG) GetWord_DM(USHORT Addr, PUSHORT Data);
   IFC(ULONG) PutWord_DM(USHORT Addr, USHORT Data);
   IFC(ULONG) PutWord_PM(USHORT Addr, ULONG Data); // байтовые опреации
   IFC(ULONG) GetWord_PM(USHORT Addr, PULONG Data); // байтовые операции
...
public:
   DaqE140(ULONG Slot) :LDaqBoard(Slot) {}
